...
const loadErrorKeyList = ["errors.unauthorized", "errors.connection", "errors.notFound"] as const;
type LoadErrorKey = (typeof loadErrorKeyList)[number];

const submitErrorKeyList = ["errors.unauthorized", "errors.connection", "errors.notFound"] as const;
...
export function EditAnimalForm({ animalId }: EditAnimalFormProps) {
  ...
  const getAnimalByIdUseCase = useMemo(
    () => appContainer.get<GetAnimalByIdUseCase>(USE_CASE_TYPES.GetAnimalByIdUseCase),
    [],
  );
  const updateAnimalUseCase = useMemo(
    () => appContainer.get<UpdateAnimalUseCase>(USE_CASE_TYPES.UpdateAnimalUseCase),
    [],
  );

  const fileInputRef = useRef<HTMLInputElement | null>(null);

  const [initialValues, setInitialValues] = useState<AddAnimalFormValues>(emptyValues);
  ...
  const formik = useFormik<AddAnimalFormValues>({
    ...
    onSubmit: async (values, helpers) => {
      ...
      try {
        ...
        if (!species || !status) {
          throw new Error("edit.errors.generic");
        }
        ...
        await updateAnimalUseCase.execute({
          animalId,
        ...
        router.push(`/${locale}/foundations/animals`);
      } catch (error) {
